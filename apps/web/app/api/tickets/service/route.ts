...
import { serviceFormSchema } from "@nova-studio/shared";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const limit = rateLimit(`service:${ip}`, 5, 60_000);
    if (!limit.success) {
    ...
    const ticket = await prisma.ticket.create({
      data: {
        ...
        submitterName: data.clientName,
        submitterEmail: data.email,
        submitterDiscord: data.discord || null,
        submitterPhone: data.phone || null,
        submitterCountry: data.country,
        verificationCode,
        verificationExpires: new Date(Date.now() + 60 * 60 * 1000),
        formData: data,
        estimatedPriceMin: body.estimatedPriceMin ? String(body.estimatedPriceMin) : null,
        estimatedPriceMax: body.estimatedPriceMax ? String(body.estimatedPriceMax) : null,
        pricingParams: {
      ...
      await fetch(`${process.env.BOT_HTTP_URL || "http://localhost:4000"}/api/notify`, {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            estimate: body.estimatedPriceMin && body.estimatedPriceMax
              ? `${body.estimatedPriceMin}€ - ${body.estimatedPriceMax}€`
              : null,
          },
        }),
      });
