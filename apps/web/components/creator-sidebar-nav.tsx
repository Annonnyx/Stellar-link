...
import { LayoutDashboard, ListChecks, FolderOpen, User, Wallet, LogOut, Sparkles } from "lucide-react";

const sidebarItems = [
  { icon: LayoutDashboard, href: "/creator", label: "Dashboard" },
  { icon: ListChecks, href: "/creator/tasks", label: "Tâches disponibles" },
  { icon: FolderOpen, href: "/creator/projects", label: "Mes projets" },
  { icon: Wallet, href: "/creator/earnings", label: "Revenus" },
  { icon: User, href: "/creator/profile", label: "Mon profil" },
];

export function CreatorSidebarNav({ creatorName }: { creatorName: string }) {
  ...
  return (
    <aside className="w-64 border-r border-border/40 bg-card flex flex-col">
      <div className="p-4">
        ...
        <p className="text-xs text-muted-foreground mt-2 truncate">{creatorName}</p>
      </div>
